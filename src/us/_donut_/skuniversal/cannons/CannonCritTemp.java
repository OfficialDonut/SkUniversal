...
public class CannonCritTemp extends SimpleExpression<Number> {
    ...
    public String toString(@Nullable Event e, boolean arg1) {
        return "the critical temperature of the cannon with ID " + id.getSingle(e);
    }
    ...
    protected Number[] get(Event e) {
        if (id.getSingle(e) != null) {
            if (Cannons.getPlugin().getCannon(UUID.fromString(id.getSingle(e))) != null) {
                return new Number[]{ Cannons.getPlugin().getCannon(UUID.fromString(id.getSingle(e))).getCannonDesign().getCriticalTemperature() };
            } else {
                return null;
            }
        } else {
