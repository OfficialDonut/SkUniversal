...
public class ExprBorderBlocks extends SimpleExpression<Block> {
    ...
    protected Block[] get(Event e) {
        ...
        Location[] corners = plot.getAllCorners().toArray(new Location[0]);
        ...
        if (plot.getAllCorners().size() == 4 && northEastCorner != null && northWestCorner != null && southEastCorner != null && southWestCorner != null) {
            ...
            for (double x = northWestCorner.getX(); x < northEastCorner.getX() + 1; x++) {
                blocks.add(new org.bukkit.Location(Bukkit.getWorld(northWestCorner.getWorld()), x, borderHeight.getSingle(e).intValue(), northWestCorner.getZ()).getBlock());
            }
            for (double z = northEastCorner.getZ(); z < southEastCorner.getZ() + 1; z++) {
                blocks.add(new org.bukkit.Location(Bukkit.getWorld(northEastCorner.getWorld()), northEastCorner.getX(), borderHeight.getSingle(e).intValue(), z).getBlock());
            }
            for (double z = northWestCorner.getZ(); z < southWestCorner.getZ() + 1; z++) {
                blocks.add(new org.bukkit.Location(Bukkit.getWorld(northWestCorner.getWorld()), northWestCorner.getX(), borderHeight.getSingle(e).intValue(), z).getBlock());
            }
            for (double x = southWestCorner.getX(); x < southEastCorner.getX() + 1; x++) {
                blocks.add(new org.bukkit.Location(Bukkit.getWorld(southWestCorner.getWorld()), x, borderHeight.getSingle(e).intValue(), southWestCorner.getZ()).getBlock());
            }
